...
namespace FsUnity.Control.Agents
...
type AutoCancelAgent<'T> private (mbox : Agent<'T>, cts : CancellationTokenSource) =
    ...
    static member Start body =
        ...
        //let cancellationToken = cts.Token TODO FIX TYPE MISMATCH
        //let underlyingAgent = Agent<'T>.Start(body, cancellationToken)
        let underlyingAgent = Agent<'T>.Start(body)
