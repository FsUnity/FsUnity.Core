    ...
    let windowed size (input : IObservable<'T>) =
        ...
        { new IObservable<'T[]> with
            member x.Subscribe observer =
                ...
                //let agent = SlidingWindowAgent<_> (size, cts.Token) TODO FIX TYPE MISMATCH
                let agent = SlidingWindowAgent<_> (size)
