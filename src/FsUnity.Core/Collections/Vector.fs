...
module Vector =
    ...
    let singleton (value : 'T) : vector<'T> =
        FsUnity.vector.UnsafeCreate [| value |]
    ...
    let blit (source : vector<'T>) sourceIndex (target : vector<'T>) targetIndex count : vector<'T> =
        ...
        FsUnity.vector.UnsafeCreate resultArray
...
//            notImpl ""
    ...
    let copy (vec : vector<'T>) : vector<'T> =
        ...
        FsUnity.vector.Create vec.Elements
    ...
    let partition (predicate : 'T -> bool) (vec : vector<'T>) : vector<'T> * vector<'T> =
        ...
        if isEmpty vec then empty, empty
            ...
            FsUnity.vector.UnsafeCreate trueVec,
            FsUnity.vector.UnsafeCreate falseVec
    ...
    let unzip (vec : vector<'T1 * 'T2>) : vector<'T1> * vector<'T2> =
        ...
        if isEmpty vec then
            ...
            FsUnity.vector.UnsafeCreate vec1,
            FsUnity.vector.UnsafeCreate vec2
    ...
    let unzip3 (vec : vector<'T1 * 'T2 * 'T3>) : vector<'T1> * vector<'T2> * vector<'T3> =
        ...
        if isEmpty vec then
            ...
            FsUnity.vector.UnsafeCreate vec1,
            FsUnity.vector.UnsafeCreate vec2,
            FsUnity.vector.UnsafeCreate vec3
    ...
    let inline ofSet (set : Set<'T>) : vector<'T> =
        ...
        FsUnity.vector.Create (Set.toArray set)
...
*)
    ...
    [<CompiledName("Unfold")>]
    let unfold (generator : 'State -> ('T * 'State) option) (state : 'State) : vector<'T> =
        // Preconditions
        // (None)
        ...
        vector.UnsafeCreate <| results.ToArray ()
    ...
    let unfoldBack (generator : 'State -> ('T * 'State) option) (state : 'State) : vector<'T> =
        // Preconditions
        // (None)
        ...
        vector.UnsafeCreate resultArray
